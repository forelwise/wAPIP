...
void _W_API_Event_Listener( WPARAM wP, LPARAM lP )
{
    switch( HIWORD( wP ) )
    {
        ...
        case EN_SETFOCUS:{
            if( wapi->_T_EDIT[ LOWORD( wP ) ] != NULL )
                if( wapi->_T_EDIT[ LOWORD( wP ) ]->onFocus != NULL )
                    wapi->_T_EDIT[ LOWORD( wP ) ]->onFocus( wapi->_T_EDIT[ LOWORD( wP ) ] );
            if( wapi->_T_EDITP[ LOWORD( wP ) ] != NULL )
                if( wapi->_T_EDITP[ LOWORD( wP ) ]->onFocus != NULL )
                    wapi->_T_EDITP[ LOWORD( wP ) ]->onFocus( wapi->_T_EDITP[ LOWORD( wP ) ] );
        }
        case EN_KILLFOCUS :{
            if( wapi->_T_EDIT[ LOWORD( wP ) ] != NULL )
                if( wapi->_T_EDIT[ LOWORD( wP ) ]->onKillFocus != NULL )
                    wapi->_T_EDIT[ LOWORD( wP ) ]->onKillFocus( wapi->_T_EDIT[ LOWORD( wP ) ] );
            if( wapi->_T_EDITP[ LOWORD( wP ) ] != NULL )
                if( wapi->_T_EDITP[ LOWORD( wP ) ]->onKillFocus != NULL )
                    wapi->_T_EDITP[ LOWORD( wP ) ]->onKillFocus( wapi->_T_EDITP[ LOWORD( wP ) ] );
        }
    }
}
...
HWND TObject::GetParent()
{
    return ::GetParent( this->hWnd );
}
void TObject::SetFont( LPSTR fontName )
{
    this->_hFont = CreateFont(0, 0, 0, 0, FW_NORMAL, false, false, false, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, FF_MODERN, fontName);
    SendMessage(this->hWnd, WM_SETFONT, WPARAM(this->_hFont), 0);
}
TObject::~TObject()
{
    DestroyWindow( this->hWnd );
...
void TObject::SetStyle( LONG style )
{
    if(SetWindowLong( this->hWnd, GWL_STYLE, style ) == 0 )
        Message( (char*)"Cannot change style" );
}
